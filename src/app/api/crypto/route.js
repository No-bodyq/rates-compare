...
import CoinbaseRateScraperService from '../../../lib/services/cryptoScrapeApi';
...
async function getScraperInstance() {
  ...
  if (!scraperInstance) {
    ...
    try {
      ...
      scraperInstance = new CoinbaseRateScraperService({
        maxRetries: 2,
        retryDelay: 3000,
      ...
      console.error('❌ Failed to initialize scraper:', error.message, error.stack);
...
export async function GET(request) {
  try {
    ...
    if (onlySendwave) {
      ...
      result = await scraper.getAllRates(base, target, amount); // Fixed method name
    }
    ...
    console.error('❌ GET Error:', error.message, error.stack); // Added detailed logging
    return NextResponse.json(
      { 
        success: false, 
...
export async function POST(request) {
  try {
    ...
    if (sendwaveOnly) {
      ...
      result = await scraper.getAllRates(base, target, amount || 1000); // Fixed method name
    }
    ...
    console.error('❌ POST Error:', error.message, error.stack); // Added detailed logging
    return NextResponse.json(
      { 
        success: false, 
        error: error.message || 'Failed to scrape rates',
...
export async function HEAD(request) {
  try {
    ...
    console.error('❌ HEAD Error:', error.message, error.stack); // Added detailed logging
    return new Response(null, { 
      status: 503,
